...
def color_similarity_lab(color1_rgb, color2_rgb):
...
def recommend_swim_caps(swimsuit_colors, cap_database, top_n=5):
    """
    수영복 색상 기반 수모 추천
    ...
    for cap in cap_database:
        ...
        for swim_color in swimsuit_colors:
            distance = color_similarity_lab(swim_color['rgb'], cap_color)
        ...
        recommendations.append({
            'cap_id': cap['id'],
            'cap_name': cap['name'],
            'cap_color_rgb': cap_color,
            'cap_color_hex': '#{:02x}{:02x}{:02x}'.format(*cap_color),
            'price': cap.get('price', 'N/A'),
