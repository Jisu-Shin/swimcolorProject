...
from yolo_utils import get_segmentation_mask, crop_swimsuit_only, apply_mask


async def example_single_image(model_path, image_path, target_class):
    """단일 이미지 시각화 예시"""
    print("=" * 60)
    ...
    # 1. 색상 추출기 초기화
    extractor = ExtractorFactory.create("ver2", str(model_path));
    ...
    all_colors = await extractor.extract_colors(image_urls, target_class)
    
    colors = all_colors[0]
    
    # 4. 마스크 및 크롭된 이미지 생성
    cropped = crop_swimsuit_only(original, extractor.model,target_class)
    mask = get_segmentation_mask(original, extractor.model, target_class)
...
async def main():
    ...
    # image_path = '/Users/zsu/MyProject/크롤링 사진/no_color_swimsuit_0220/0087_배럴_칼리 브이백.jpg'
    cap_image_path = '/Users/zsu/MyProject/크롤링 사진/no_color_swimcap_0220/0014_루프루프_리본 타이 .jpg'


    await example_single_image(model_path, cap_image_path, 0)
